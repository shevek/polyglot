...
public class PolyglotEngine {
    ...
    private void dump(@CheckForNull CharSink textSink, @CheckForNull CharSink dotSink, @Nonnull GrammarModel grammar, @Nonnull LRAutomaton automaton) throws IOException {
    ...
    @Nonnull
    protected void buildLexer(@Nonnull GrammarModel grammar) throws IOException {
        Stopwatch stopwatch = Stopwatch.createStarted();
    ...
    protected void buildFunctions(@Nonnull GrammarModel grammar) throws IOException {
        ...
        if (sink != null) {
            try (Writer writer = sink.openBufferedStream()) {
                ...
                FollowFunction followFunction = new FollowFunction(grammar, firstFunction);
                for (CstProductionModel production : grammar.cstProductions.values())
    ...
    @Nonnull
    protected LRAutomaton buildParserLr0(@Nonnull PolyglotExecutor executor, @Nonnull GrammarModel grammar) throws IOException, InterruptedException, ExecutionException {
        Stopwatch stopwatch = Stopwatch.createStarted();

        LR0ItemUniverse universe = new LR0ItemUniverse(grammar);
        ...
        dump(debugHandler.forTarget(AUTOMATON_LR0_DESC, ".lr0.txt"), debugHandler.forTarget(AUTOMATON_LR0, ".lr0.dot"), grammar, automaton);
    ...
    @Nonnull
    protected Tables buildTables(@Nonnull GrammarModel grammar, @CheckForNull LRAutomaton automaton) throws IOException {
        Stopwatch stopwatch = Stopwatch.createStarted();

        Tables.Builder builder = new Tables.Builder(grammar, automaton, isOption(Option.INLINE_TABLES));
        Tables tables = builder.build();

        LOG.info("{}: Building tables took {} and built {}", getName(), stopwatch, tables);
        ...
        try {
            ...
            if (grammar.cstProductionRoot != null) {
                ...
                if (isOption(Option.SLR)) {
                    automaton = buildParserLr0(executor, grammar);
                ...
                if (isOption(Option.LR1)) {
                    automaton = buildParserLr1(executor, grammar);
                    conflicts = automaton.getConflicts();
                    if (conflicts.isEmpty())
                        break AUTOMATON;
                    if (errors.isFatal())
